...
def AlSetAlarm(query):
    ...
    pythonPath = subprocess.check_output('python -c "import sys; ' +
                                         'print(sys.executable)"')
    pythonPath = pythonPath.decode("utf-8").replace('\r\n', '')
...
speak('At what time you want to set the alarm? Give time command in 12 hour ' +
      'format.')
